...
import { Layers, Compass, Crosshair, ZoomIn, ZoomOut, Flame, Route, X, ShieldCheck, ShieldAlert, AlertTriangle } from 'lucide-react';
    ...
    validAlerts.forEach(alert => {
      ...
      popupContent.innerHTML = `
        ...
        <div class="pt-2 border-t border-slate-200 dark:border-slate-700/80 flex items-center justify-between text-[10px]">
          ...
          <button id="popup-btn-${escapeXml(alert.id)}" class="px-2 py-1 bg-blue-600 hover:bg-blue-500 text-white rounded font-bold transition-colors">
            Detalles
          </button>
        </div>
      `;

      popupContent.querySelector(`#popup-btn-${CSS.escape(alert.id)}`)?.addEventListener('click', (e) => {
        e.stopPropagation();
        onSelectAlert(alert);
      });
