...
// Proveedor de IA configurable (cualquier API compatible con OpenAI /v1/chat/completions).
// Por defecto apunta a Groq; puedes cambiarlo a OpenAI, OpenRouter, Together, etc.
const GROQ_API_KEY = process.env.AI_API_KEY || process.env.GROQ_API_KEY || '';
const GROQ_BASE = process.env.AI_BASE_URL || 'https://api.groq.com/openai/v1/chat/completions';
const GROQ_MODELS = (process.env.AI_MODELS || process.env.GROQ_MODELS || 'openai/gpt-oss-120b,openai/gpt-oss-20b')
  .split(',')
  .map((m) => m.trim())
  .filter(Boolean);
...
// Límite de tasa en memoria para proteger la cuota del proveedor de IA.
...
// Shared resilient AI call helper with multi-model fallback & retries (OpenAI-compatible)
async function callAIWithFallback(
  prompt: string,
  options?: { responseJson?: boolean }
): Promise<string | null> {
  if (!GROQ_API_KEY) {
    console.warn('AI_API_KEY/GROQ_API_KEY no configurada; usando heurística local.');
    return null;
  }
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      try {
        ...
        if ((err as Error)?.name === 'AbortError') {
          console.warn(`[AI ${model}] timeout (30s)`);
          break;
        }
        console.warn(`[AI ${model}] intento ${attempt + 1}: ${(err as Error)?.message}`);
...
// 1. API: Geolocate
app.post('/api/geolocate', requireAccessToken, enRateLimit, async (req, res) => {
  try {
    const { text } = req.body;
...
Si no encuentras una ubicación en México, devuelve lat: null, lon: null.`;
    ...
    const rawResponse = await callAIWithFallback(prompt, { responseJson: true });
    if (rawResponse) {
      try {
...
// 2. API: Classify (usa IA cuando hay API key configurada; si no, cae a heurística local)
app.post('/api/classify', enRateLimit, async (req, res) => {
  try {
    const { text } = req.body;
    if (!text || typeof text !== 'string') {
      return res.status(400).json({ error: 'Text parameter is required' });
    ...
    // 1) Intentar IA (OpenAI-compatible) para una clasificación más inteligente.
    if (GROQ_API_KEY) {
      try {
        const prompt = `Clasifica la siguiente alerta vial/seguridad de México en UNA de estas categorías: red (bloqueos, cierres, protestas, retenes), orange (incidentes, choques, accidentes, congestión), green (vía libre, flujo normal), security (violencia, balaceras, secuestro, robo). Responde SOLO con una palabra (lowercase), sin puntuación ni markdown.

Alerta: "${trimmed.replace(/"/g, "'")}"`;
        const rawResponse = await callAIWithFallback(prompt);
        if (rawResponse) {
          const candidate = rawResponse.trim().toLowerCase().match(/red|orange|green|security/)?.[0];
          if (candidate) type = candidate;
        }
      } catch {
        // si la IA falla, se usa la heurística local
      }
    }

    // 2) Fallback heurístico local (sin API key o si la IA no produjo categoría válida)
    if (type === 'orange') {
    ...
    const raw = [
      { label: type, score: type === 'orange' ? 0.6 : 0.9 },
      { label: type === 'red' ? 'bloqueo' : type === 'orange' ? 'incidente' : type === 'green' ? 'via_libre' : 'zona_roja', score: 0.9 }
    ];
...
app.post('/api/classify-batch', requireAccessToken, enRateLimit, async (req, res) => {
  try {
...
Usa "unknown" cuando no se pueda determinar estado/ciudad/carretera.`;
    ...
    const validTypes = ['red', 'orange', 'green', 'security'];
    const results = texts.map((_: unknown, i: number) => {
      const p = parsed?.[String(i)] || {};
      const type = validTypes.includes(p.type as string) ? (p.type as string) : 'orange';
      return {
        index: i,
        type,
...
// 3. API: NER
app.post('/api/ner', enRateLimit, async (req, res) => {
  try {
    const { text } = req.body;
...
app.get('/api/feed-proxy', enRateLimit, async (req, res) => {
  ...
  if (feedUrl && isSafePublicUrl(feedUrl)) {
    try {
      ...
      // Evitar SSRF por redirección: solo seguir si el destino también es una URL pública segura.
      if (response.status >= 300 && response.status < 400) {
        const location = response.headers.get('location');
        if (location && isSafePublicUrl(new URL(location, feedUrl).toString())) {
          const followController = new AbortController();
          const followTimeout = setTimeout(() => followController.abort(), 6000);
          const followed = await fetch(new URL(location, feedUrl).toString(), {
            signal: followController.signal,
            redirect: 'error',
            headers: { 'User-Agent': 'CentralVialMX-Bot/1.0 (Highway Traffic Monitor)', 'Accept': 'application/json, text/xml, application/xml, */*' }
          });
          clearTimeout(followTimeout);
          return responderFeed(followed, res);
        }
        return res.json({ title: 'NEXO — Red Vial', status: 'feed_unavailable', items: [], error: 'Redirección no segura' });
      }

      return responderFeed(response, res);
    } catch {
...
// Procesa la respuesta del feed (HTTP Response) y responde con los items normalizados.
async function responderFeed(response: Response, res: express.Response): Promise<void> {
  if (response.ok) {
    ...
    if (/xml|rss|atom/i.test(contentType)) {
      ...
      if (parsed) {
        res.json({ title: 'NEXO — Red Vial', status: 'rss_synced', items: parsed });
        return;
      }
    } else {
      try {
  ...
  res.json({
    ...
    status: 'feed_unavailable',
    items: [],
    error: 'No se pudo obtener el feed RSS'
  });
}

// 5. API: Deep AI Alert Inspection
app.post('/api/ai-analyze-alert', requireAccessToken, enRateLimit, async (req, res) => {
  try {
    const { text } = req.body;
